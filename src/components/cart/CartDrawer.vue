  ...
  <Teleport to="body">
    <transition name="slide">
      <div v-if="isOpen" class="fixed inset-0 z-[10000]">
        ...
        <div class="absolute inset-y-0 right-0 w-full max-w-md bg-white dark:bg-[#080808] shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col h-full z-[10010]">
          
          <div class="p-8 border-b dark:border-white/10 flex justify-between items-center bg-white dark:bg-[#080808] shrink-0">
            <div class="flex items-center gap-4">
              <button v-if="step === 2" @click="step = 1" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 dark:bg-white/5 text-[#DE1F27] hover:scale-110 transition-all">
                <span class="text-xl font-bold">←</span>
              </button>
              <div>
                <h2 class="text-3xl font-[1000] text-[#152C77] dark:text-white uppercase italic leading-none tracking-tighter">
            ...
            <button @click="closeAndReset" class="w-10 h-10 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">✕</button>
          </div>
          ...
          <div class="flex-grow p-6 overflow-y-auto no-scrollbar bg-white dark:bg-[#080808]">
            ...
            <div v-if="step === 1" class="space-y-8">
              <div v-if="cartStore.items.length > 0" class="space-y-6">
                <div v-for="item in cartStore.items" :key="item.id" class="flex gap-4 group">
                  <div class="w-20 h-20 bg-slate-50 dark:bg-white/5 rounded-2xl p-2 shrink-0 border border-slate-100 dark:border-white/10 relative">
            ...
            <div v-if="step === 2" class="p-2 space-y-6 animate-fade-in">
              <div class="space-y-5">
                <div v-for="field in ['nombre', 'telefono', 'direccion']" :key="field" class="flex flex-col">
                  <label class="text-[9px] font-[1000] text-slate-400 uppercase tracking-[0.2em] mb-2 ml-1">{{ field === 'telefono' ? 'WhatsApp' : field }}</label>
                  <input v-model="form[field]" type="text" class="custom-input" :placeholder="`Tu ${field}...`" />
                </div>
                <div class="flex flex-col">
                  <label class="text-[9px] font-[1000] text-slate-400 uppercase tracking-[0.2em] mb-2 ml-1">Zona de Entrega</label>
          ...
          <div v-if="cartStore.items.length > 0" 
            ...
            <button v-else @click="handleFinalizeOrder" :disabled="!isFormValid"
              ...
              <span>{{ loading ? 'Creando Orden...' : 'Enviar a WhatsApp' }}</span>
...
// MOCK AUTH (Simulado para que no de error)
const authStore = reactive({
  isLoggedIn: true, 
  user: { nombre: 'Natalia Fuentes', telefono: '3124567890', direccion: 'Calle 123', zona: 'Chía' }
});
...
const form = reactive({ nombre: '', telefono: '', direccion: '', zona: '' });
...
const isFormValid = computed(() => form.nombre && form.telefono && form.direccion && form.zona);
...
const sendWhatsApp = (orderId) => {
  ...
  let message = `🚀 *ORDEN #${orderId} - PET STATION*\n`;
  message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
  message += `👤 *Cliente:* ${form.nombre}\n`;
  message += `📍 *Entrega:* ${form.direccion} (${form.zona})\n`;
  message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  cartStore.items.forEach(i => { message += `• ${i.nombre} (x${i.quantity})\n`; });
  message += `\n💰 *TOTAL: $${cartStore.totalPrice.toLocaleString()}*\n`;
  message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
