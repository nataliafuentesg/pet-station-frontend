<template>
  <div class="relative w-full max-w-md bg-ps-black rounded-[3rem] p-10 shadow-2xl border border-white/5 animate-in zoom-in duration-300">
    ...
    <form @submit.prevent="handleLogin" class="space-y-6">
      ...
      <div class="space-y-1">
        ...
        <input v-model="form.password" type="password" required placeholder="••••••••" class="input-dark" />
      </div>
    ...
    <div class="mt-8 pt-6 border-t border-white/5 text-center">
      <button @click="$emit('goRegister')" class="text-[10px] font-black uppercase text-white/30 hover:text-ps-red tracking-widest transition-all italic cursor-pointer">
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      emit('notify', { msg: data.message || "Error al entrar", type: 'error' });
    }
  } catch (e) {
    emit('notify', { msg: "Servidor desconectado", type: 'error' });
  } finally {
    loading.value = false;
  }
};
