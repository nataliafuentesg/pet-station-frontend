    ...
    <div class="w-full max-w-[400px] bg-slate-50 dark:bg-[#0A0A0A] rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden relative z-[10]">
      ...
      <div class="p-8 flex flex-col items-center text-center">
        ...
        <div class="w-full bg-[#152C77] dark:bg-white/5 p-6 rounded-[2rem] border border-white/10 shadow-inner mb-8 relative overflow-hidden">
          ...
          <div class="text-3xl md:text-4xl font-[1000] text-white tracking-[0.15em] italic uppercase leading-none relative z-10">
            GAITAPETS
          </div>
        </div>

        <button @click="showTermsModal = true" class="w-full bg-[#DE1F27] text-white py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest italic shadow-xl shadow-red-500/30 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 mb-4">
        ...
        <div class="w-full text-left bg-slate-100 dark:bg-white/5 p-4 rounded-xl border border-slate-200 dark:border-white/5">
          ...
          <ul class="text-[8px] text-slate-500 dark:text-slate-400 list-disc pl-4 space-y-1.5 leading-relaxed font-bold">
            <li><strong>20% de descuento</strong> en el servicio.</li>
            <li>Exclusivo para <strong>clientes nuevos</strong> (primera visita).</li>
            <li>El valor base del servicio varía según el estado del manto, largo del pelo y especie.</li>
          </ul>
        </div>
      ...
      <div v-if="showTermsModal" class="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div class="bg-white dark:bg-[#0A0A0A] w-full max-w-[450px] rounded-[2rem] p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col max-h-[90vh]">
          ...
          <div class="flex justify-between items-center mb-4 shrink-0">
            <h3 class="text-xl font-[1000] uppercase italic text-[#152C77] dark:text-white">Reglas del Juego 🐾</h3>
          ...
          <div class="overflow-y-auto custom-scrollbar pr-2 mb-6">
            <ul class="text-[11px] text-slate-600 dark:text-slate-300 list-disc pl-4 space-y-3 font-medium">
              <li>Recibe un <strong>20% de descuento</strong> sobre el valor total del servicio. Exclusivo para <strong>clientes nuevos</strong> y válido por una (1) sola vez por mascota.</li>
...
const aceptarYRedimir = () => {
  ...
  const msg = encodeURIComponent("¡Hola Pet Station! 🐾 Soy de la comunidad de @lagaita. Ya leí y acepté las condiciones y quiero usar mi código GAITAPETS para agendar mi primera visita con el 20% de descuento.");
